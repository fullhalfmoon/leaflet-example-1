...
(function (window, document, undefined) {
	...
	const MODIFIED_MARK = '◆';
	const BUTTON_CSVFILE ='select-csvfile'
	const BUTTON_CLEAR ='clear-data'
		...
		data.forEach(item => {
			...
			switch(item.type) {
				case 'marker':  // Marker
					...
					let marker = L.marker(item.latLng,{draggable:'true'}).addTo(group);
					marker.setIcon(L.spriteIcon(markerColor));
	...
	document.getElementById(BUTTON_CSVFILE).onchange = readFile;
	function readFile(e) {
	...
	// 保存データ削除ボタンのイベントハンドラーを設定する
	document.getElementById(BUTTON_CLEAR).onclick = clearData;
